      ...
      while(1)
      {
        do 
        {
          ...
          if ((!status) && (NewDataReady != 0)) 
          {
            ...
            closest_white_distance = 9999; //reset closest white distance from last iteration. 
            closest_black_distance = 9999; //reset closest black distance from last iteration. 

            for(i = 16; i < 48; i++) //find closest white and black zone's distances and angles
            {
                //find closest white zone
                if(target_info.target_distance[i] < closest_white_distance && target_info.target_reflectance[i] > refelction_threshold)
                {
                    closest_white_distance = target_info.target_distance[i];
                    closest_white_angle = target_info.target_angle[i];
                }
                //find closest black zone
                if(target_info.target_distance[i] < closest_black_distance && target_info.target_reflectance[i] < refelction_threshold)
                {
                    closest_black_distance = target_info.target_distance[i];
                    closest_black_angle = target_info.target_angle[i];
                }
            }
            SerialPort.print("\n");
            SerialPort.print("\n");
